...
- **Composition:** the system's hardware is the hero subject in an atmospheric scene evoking that platform's era (no copyrighted character likenesses if redistribution license is a concern — favour generic motifs).
- **Left-darkening:** the left ~45% must be dark enough for white text (the theme adds a left scrim, but bake in extra falloff).
...
Tie each poster's dominant light to the system's `${systemColor}` (provided per row in the manifest) so the art and hero background tint agree. Choose from 8 mood families: warm torchlight, cool fog, magenta neon, emerald CRT, amber sunset, violet synthwave, teal abyss, crimson arcade. Pick the family nearest the system color.

## Generation workflow

1. Generate 5–8 anchors; lock a style reference + weight.
2. Batch per-system prompts from `docs/hero-art/prompts.tsv` (see `scripts/gen-hero-art-prompts.py`) through your image model with that style ref.
